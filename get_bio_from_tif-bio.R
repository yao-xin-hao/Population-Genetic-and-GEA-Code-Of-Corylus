...
bio_list <- list()
for (i in 1:19) {
  bio_list[[i]] <- raster(paste0("D:\\arcgis\\wc2.1_2.5m_bio\\wc2.1_2.5m_bio_", i, ".tif"))
}
...
for (i in 1:19) {
  bio_results[[paste0("bio", i)]] <- extract(bio_list[[i]], locations)
}

# 保存为 TXT 文件
write.table(bio_results, "bio_results.txt", sep = ",", quote = FALSE, row.names = FALSE, col.names = TRUE)

print("✅ 数据提取完成，已保存为 bio_results.txt")
